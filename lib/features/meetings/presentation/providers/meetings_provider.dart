...
final meetingsRepositoryProvider = Provider<MeetingsRepository>((_) => MeetingsRepository());
...
final meetingDetailsProvider = FutureProvider.family<Meeting, String>((ref, meetingId) async {
  final user = ref.watch(currentUserProvider);
...
final meetingDecisionsProvider = FutureProvider.family<List<Decision>, String>((ref, meetingId) async {
  final user = ref.watch(currentUserProvider);
  if (user == null) return [];
...
final meetingTasksProvider = FutureProvider.family<List<MeetingTask>, String>((ref, meetingId) async {
  final user = ref.watch(currentUserProvider);
  if (user == null) return [];
  return ref.watch(meetingsRepositoryProvider).getMeetingTasks(user.id, meetingId);
});
...
class MeetingActionsNotifier extends StateNotifier<AsyncValue<void>> {
  ...
  MeetingActionsNotifier(this._repo, this._userId) : super(const AsyncValue.data(null));
...
final meetingActionsProvider = StateNotifierProvider<MeetingActionsNotifier, AsyncValue<void>>((ref) {
  final user = ref.watch(currentUserProvider);
  return MeetingActionsNotifier(ref.watch(meetingsRepositoryProvider), user?.id ?? '');
});
